...
export class HealthController {
  ...
  getAllEvents(@Query('limit') limit?: string, @Query('batchId') batchId?: string) {
    return this.healthService.getAllHealthEvents(
  ...
  deleteEvent(@Param('id') id: string) {
    return this.healthService.deleteHealthEvent(id);
  }
  ...
  getVisitors(@Query('days') days?: string) {
    return this.healthService.getVisitors(days ? parseInt(days, 10) : 30);
  }

  @Patch('visitors/:id/checkout')
  @RequirePermission(Permission.HEALTH_VISITOR_LOG)
  checkOut(@Param('id') id: string) {
    return this.healthService.checkOutVisitor(id);
  }
  ...
  @RequirePermission(Permission.HEALTH_VISITOR_LOG)
  createAdvanceNotice(@Body() body: any, @CurrentUser() user: any) {
    return this.healthService.createAdvanceNotice(body, user.id);
  }

  @Get('visitors/advance')
  @RequirePermission(Permission.HEALTH_VIEW)
  getAdvanceNotices(@Query('days') days?: string) {
    return this.healthService.getAdvanceNotices(days ? parseInt(days, 10) : 30);
  }
  ...
  // ── Health Checklist (HL-01) ─────────────────────────────────────────────

  @Post('checklist')
  @RequirePermission(Permission.HEALTH_VIEW)
  submitChecklist(@Body() body: any, @CurrentUser() user: any) {
    return this.healthService.submitChecklist(body, user.id);
  }

  @Get('checklist')
  @RequirePermission(Permission.HEALTH_VIEW)
  getChecklists(@Query('days') days?: string) {
    return this.healthService.getChecklists(days ? parseInt(days, 10) : 14);
  }

  @Get('checklist/:id')
  @RequirePermission(Permission.HEALTH_VIEW)
  getChecklist(@Param('id') id: string) {
    return this.healthService.getChecklistById(id);
  }

  // ── Biosecurity Checkpoint (HL-02) ───────────────────────────────────────

  @Post('biosecurity')
  @RequirePermission(Permission.HEALTH_VISITOR_LOG)
  submitBiosecurity(@Body() body: any, @CurrentUser() user: any) {
    return this.healthService.submitBiosecurityLog(body, user.id);
  }
  ...
  getBiosecurity(
    ...
    return this.healthService.getBiosecurityLogs(
