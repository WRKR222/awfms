import {
  ...
  export class StoreController {
    ...
    summary() {
      return this.svc.getStoreSummary();
    }
  
    @Get('pending-intakes')
    @RequirePermission(Permission.INVENTORY_VIEW)
    pendingIntakes() {
      return this.svc.getPendingIntakes();
    }
  
    @Post('egg-intake')
    @RequirePermission(Permission.INVENTORY_MANAGE)
    createIntake(@Body() dto: CreateStoreIntakeDto, @Request() req: any) {
      return this.svc.createEggIntake(dto, req.user);
    }
    ...
    getIntakes(
      ...
      @Query('intakeDate') intakeDate?: string,
    ) {
    ...
    getIntake(@Param('id') id: string) {
      return this.svc.getIntakeById(id);
    }
  
    @Post('feed-distribution')
    @RequirePermission(Permission.FEED_INTAKE_LOG)
    logFeed(@Body() dto: LogFeedDistributionDto, @Request() req: any) {
      return this.svc.logFeedDistribution(dto, req.user);
    }
