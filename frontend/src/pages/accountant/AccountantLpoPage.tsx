...
import {
  ClipboardList, FileText, Eye, X, CheckCircle2, XCircle, Send, Plus, Trash2, Download,
} from 'lucide-react';
    ...
    const printContent = `
      <html><head>
        ...
        <style>
          ...
          .shipped{background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:10px 14px;margin-bottom:16px;font-size:13px}
          table{width:100%;border-collapse:collapse;font-size:13px}
          th{text-align:left;border-bottom:2px solid #e5e7eb;padding:6px 4px;font-size:11px;text-transform:uppercase;color:#6b7280}
        ...
        <p class="meta">LPO # <strong>${lpo.lpoNumber}</strong> &nbsp;|&nbsp; Date: ${dayjs(lpo.lpoDate).format('D MMMM YYYY')} &nbsp;|&nbsp; Status: ${lpo.status}</p>
        <div class="shipped">
          <strong>Shipped To:</strong> Anza Whole Foods
        </div>
        <p style="margin-bottom:4px;font-size:13px"><strong>Supplier:</strong> ${lpo.supplierName}</p>
        ${lpo.expectedDelivery ? '<p style="font-size:13px;margin-bottom:4px"><strong>Expected Delivery:</strong> ' + dayjs(lpo.expectedDelivery).format('D MMMM YYYY') + '</p>' : ''}
        ${lpo.purchaseRequest ? '<p style="font-size:12px;color:#555;margin-bottom:12px">PR Ref: ' + lpo.purchaseRequest.requestRef + '</p>' : '<br/>'}
        <table>
          <thead><tr><th>Item / Description</th><th class="right">Qty</th><th class="right">Unit Price</th><th class="right">Subtotal</th></tr></thead>
          <tbody>
            ${(lpo.items as any[]).map(i => {
              const qty = Number(i.quantity ?? 0);
              const up  = Number(i.unitPrice ?? 0);
              return '<tr><td>' + (i.storeItem?.name ?? i.description ?? '—') + '</td><td class="right">' + qty + ' ' + (i.storeItem?.unit ?? '') + '</td><td class="right">KES ' + up.toLocaleString() + '</td><td class="right">KES ' + Number(i.subtotal ?? qty * up).toLocaleString() + '</td></tr>';
            }).join('')}
          </tbody>
          <tfoot>
            <tr><td colspan="3" class="right" style="padding-top:8px">Subtotal</td><td class="right" style="padding-top:8px">KES ${Number(lpo.subtotalKes).toLocaleString()}</td></tr>
            <tr><td colspan="3" class="right">VAT</td><td class="right">KES ${Number(lpo.vatKes).toLocaleString()}</td></tr>
            <tr class="total-row"><td colspan="3" class="right">Grand Total</td><td class="right">KES ${Number(lpo.totalKes).toLocaleString()}</td></tr>
          </tfoot>
        </table>
        ${lpo.notes ? '<p style="margin-top:16px;font-size:12px;color:#555">Notes: ' + lpo.notes + '</p>' : ''}
        <div class="footer">Generated by AWFMS &mdash; Anza Whole Foods Farm Management System</div>
      </body></html>
    `;
  ...
  return (
    <Modal title={lpo.lpoNumber} onClose={onClose}>
      <div className="space-y-3 text-sm">
        ...
        {/* Shipped To */}
        <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700 rounded-xl px-3 py-2 text-xs">
          <span className="text-gray-500 dark:text-gray-400">Shipped To: </span>
          <span className="font-semibold text-green-700 dark:text-green-400">Anza Whole Foods</span>
        </div>
        ...
        <div className="flex justify-end pt-2">
          <button
            onClick={handlePrint}
            className="flex items-center gap-2 bg-brand-green text-white px-4 py-2 rounded-xl text-sm font-semibold hover:bg-green-700 transition-colors"
          >
