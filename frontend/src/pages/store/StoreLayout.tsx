...
import { Warehouse, Home, Egg, Truck, LogOut, Settings2, ClipboardList, Bell, Settings, Package } from 'lucide-react';
...
const navDefs = [
  ...
  { label: 'Operations',  icon: Settings2,     to: '/store/operations' },
  { label: 'Issuance',    icon: ClipboardList, to: '/store/issuance' },
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-dark-bg flex flex-row">
          ...
          <div className="flex flex-col gap-1 border-t border-gray-100 dark:border-dark-border pt-4">
            ...
            <div className="flex items-center gap-3 px-1 py-2">
              ...
              <motion.div animate={{ display: open ? 'block' : 'none', opacity: open ? 1 : 0 }} className="overflow-hidden min-w-0">
                ...
                <p className="text-xs text-gray-400 dark:text-gray-500">Store</p>
              </motion.div>
            </div>
          </div>
        </SidebarBody>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="md:hidden flex items-center justify-between px-4 py-3 bg-brand-green text-white sticky top-0 z-10">
          <div className="flex items-center gap-3 min-w-0"><MobileSidebar links={sidebarLinks} user={user ?? undefined} roleLabel={"Store"} onLogout={handleLogout} /><div className="min-w-0">
            <p className="text-xs opacity-75">Store</p>
            <p className="font-semibold text-sm">{user?.fullName}</p>
          </div></div>
          <button onClick={() => navigate('/store/notifications')} className="relative p-1">
            <Bell className="w-5 h-5" />
            {unreadCount > 0 && (
