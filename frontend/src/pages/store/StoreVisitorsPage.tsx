...
// for Director approval (same /visitors API as the Manager role).
//
// HOW TO WIRE UP:
      ...
      {showForm && (
        <form onSubmit={handleSubmit(d => createMut.mutate(d))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <Fld label="Visitor Name *" err={errors.name?.message}>
              <input {...register('name', { required: 'Required' })} className="input" placeholder="Driver / Representative name" />
            </Fld>
            <Fld label="Phone">
              <input {...register('phone')} className="input" />
            </Fld>
            <Fld label="Company / Supplier">
              <input {...register('company')} className="input" placeholder="e.g. Unga Feeds Ltd" />
            </Fld>
            <Fld label="ID Number">
            ...
            <Fld label="Expected Arrival Date/Time">
              <input type="datetime-local" {...register('expectedArrival')} className="input" />
            </Fld>
          ...
          {createMut.isError && <p className="text-xs text-red-600">Failed to submit. Please try again.</p>}
        </form>
      )}
              ...
              <tbody>
                {list.map(v => (
                  <tr key={v.id} className="border-t border-gray-100 dark:border-dark-border hover:bg-gray-50 dark:hover:bg-gray-800/50">
                    ...
                    <td className="px-4 py-2 text-gray-600">{v.company ?? '—'}</td>
                    <td className="px-4 py-2 text-gray-600 max-w-[200px] truncate">{v.purpose}</td>
                    <td className="px-4 py-2 text-gray-500 whitespace-nowrap">
                      {v.expectedArrival ? dayjs(v.expectedArrival).format('DD/MM/YY HH:mm') : '—'}
                    </td>
                    <td className="px-4 py-2 text-center">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${statusColor(v.approvalStatus)}`}>
                        {v.approvalStatus}
                      </span>
                    </td>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setViewing(null)}>
          <div className="bg-white dark:bg-dark-card rounded-2xl p-6 w-full max-w-md space-y-3" onClick={e => e.stopPropagation()}>
            <div className="flex items-start justify-between">
              <h2 className="font-bold text-lg text-gray-800 dark:text-gray-100">{viewing.name}</h2>
              <button onClick={() => setViewing(null)}><X className="w-5 h-5 text-gray-400" /></button>
            </div>
            {([
              ['Company', viewing.company], ['Phone', viewing.phone], ['ID Number', viewing.idNumber],
              ['Purpose', viewing.purpose],
              ['Expected', viewing.expectedArrival ? dayjs(viewing.expectedArrival).format('DD/MM/YYYY HH:mm') : null],
              ['Notes', viewing.notes], ['Approval Status', viewing.approvalStatus],
