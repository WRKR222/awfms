...
import {
  ...
  Users, BarChart2, Heart, Settings, AlertTriangle, Syringe, ClipboardList, TrendingUp,
} from 'lucide-react';
...
export function ManagerLayout() {
  ...
  const sidebarLinks: SidebarLinkItem[] = [
    ...
    { to: '/manager/hdp-controls',      label: 'HDP% Controls', icon: <TrendingUp className="w-5 h-5" /> },
    { to: '/manager/requisitions',      label: 'Store Requests', icon: <ClipboardList className="w-5 h-5" /> },
  ...
  const mobileNav = sidebarLinks.slice(0, 5);
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-dark-bg flex flex-row">
          ...
          <div className="flex flex-col gap-1 border-t border-gray-100 dark:border-dark-border pt-4">
            ...
            <div className="flex items-center gap-3 px-1 py-2">
              <div className="w-7 h-7 rounded-full bg-brand-green/20 flex items-center justify-center flex-shrink-0">
                <span className="text-xs font-bold text-brand-green">{user?.fullName?.charAt(0) ?? 'M'}</span>
              </div>
              <motion.div
                animate={{ display: open ? 'block' : 'none', opacity: open ? 1 : 0 }}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="md:hidden flex items-center justify-between px-4 py-3 bg-brand-green text-white sticky top-0 z-10">
          ...
          <button onClick={() => navigate('/manager/notifications')} className="relative p-1">
            <Bell className="w-5 h-5" />
            {unreadCount > 0 && (
