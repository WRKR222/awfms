...
const navDefs = [
  ...
  { label: 'Orders',   icon: ShoppingCart, to: '/sales/orders' },
  { label: 'Bookings', icon: BookOpen,     to: '/sales/bookings' },
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-dark-bg flex flex-row">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="md:hidden flex items-center justify-between px-4 py-3 bg-brand-green text-white sticky top-0 z-10">
          <div className="flex items-center gap-3 min-w-0"><MobileSidebar links={sidebarLinks} user={user ?? undefined} roleLabel={"Sales"} onLogout={handleLogout} /><div className="min-w-0"><p className="text-xs opacity-75">Sales</p><p className="font-semibold text-sm">{user?.fullName}</p></div></div>
          <button onClick={() => navigate('/sales/notifications')} className="relative p-1"><Bell className="w-5 h-5" />{unreadCount > 0 && <span className="absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full w-4 h-4 flex items-center justify-center">{unreadCount > 9 ? '9+' : unreadCount}</span>}</button>
        </header>
