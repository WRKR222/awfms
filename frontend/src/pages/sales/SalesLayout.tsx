...
import { ShoppingCart, Home, BookOpen, Users, Bell, LogOut, FileCheck, Settings, EggOff } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-dark-bg flex flex-row">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="md:hidden flex items-center justify-between px-4 py-3 bg-brand-green text-white sticky top-0 z-10">
          <div className="flex items-center gap-3 min-w-0"><MobileSidebar links={sidebarLinks} user={user ?? undefined} roleLabel={"Sales"} onLogout={handleLogout} /><div className="min-w-0">
            <p className="text-xs opacity-75">Sales</p>
            <p className="font-semibold text-sm">{user?.fullName}</p>
          </div></div>
          <button onClick={() => navigate('/sales/notifications')} className="relative p-1">
            <Bell className="w-5 h-5" />
            {unreadCount > 0 && (
