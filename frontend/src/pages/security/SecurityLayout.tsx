...
import { ShieldCheck, Home, Users, Bell, LogOut } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-dark-bg flex flex-row">
          ...
          <div className="flex flex-col gap-1 border-t border-gray-100 dark:border-dark-border pt-4">
            ...
            <div className="flex items-center gap-3 px-1 py-2">
              <div className="w-7 h-7 rounded-full bg-brand-green/20 flex items-center justify-center flex-shrink-0">
                <span className="text-xs font-bold text-brand-green">{user?.fullName?.charAt(0) ?? 'S'}</span>
              </div>
              <motion.div animate={{ display: open ? 'block' : 'none', opacity: open ? 1 : 0 }} className="overflow-hidden min-w-0">
                ...
                <p className="text-xs text-gray-400 dark:text-gray-500">{gateLabel}</p>
              </motion.div>
            </div>
          </div>
        </SidebarBody>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="md:hidden flex items-center justify-between px-4 py-3 bg-brand-green text-white sticky top-0 z-10">
          <div className="flex items-center gap-3 min-w-0"><MobileSidebar links={sidebarLinks} user={user ?? undefined} roleLabel={gateLabel} onLogout={handleLogout} /><div className="min-w-0">
            <p className="text-xs opacity-75">{gateLabel}</p>
            <p className="font-semibold text-sm">{user?.fullName}</p>
          </div></div>
          <button onClick={() => navigate(`${base}/notifications`)} className="relative p-1">
            <Bell className="w-5 h-5" />
            {unreadCount > 0 && (
