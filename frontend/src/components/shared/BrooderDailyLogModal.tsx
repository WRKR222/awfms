...
import {
  ...
  Grid3x3, Eye, Loader2, CheckCircle2,
} from 'lucide-react';
...
export function BrooderDailyLogModal({ batch, presetScope, onClose }: Props) {
  ...
  // ── Cage Reassignment, text-description mode ──────────────────────────
  // Alternative to the block builder above — describe the layout in plain
  // text (e.g. "Row F Level 4: 1-33: 9 birds each, 34: 8 birds, ...") and
  // let the server parse it, instead of computing block boundaries by hand.
  // Handles mixed per-cage counts within one contiguous run, which the
  // block builder can only do by adding a separate block per distinct
  // count. See POST /brooder/batches/:batchId/reassign-bulk-text[/preview].
  ...
  // Any edit after a preview invalidates it — don't let a stale "looks
  // good" preview linger once the description or date has changed.
  useEffect(() => { setReassignPreview(null); }, [reassignDescription, logDate]);
  ...
  const submit = useMutation({
    mutationFn: async () => {
      ...
      // Independent of every other section. Two ways to describe the new
      // layout: the block builder (rowId/levelIds/cageCount/birdsPerCage),
      // or a plain-text description parsed server-side — same destination
      // endpoint family either way.
      let reassignResult: any = null;
      if (reassignMode === 'text' && reassignDescription.trim().length > 0) {
        try {
      ...
      } else if (reassignMode === 'blocks' && reassignBlocks.length > 0) {
        try {
  ...
  function handleSubmit() {
    ...
    if (reassignMode === 'text' && reassignDescription.trim().length > 0 && !reassignPreview) {
      setSubmitError('Preview the cage layout description before saving, so you can confirm it parsed the way you expect.');
      return;
    }
    if (reassignMode === 'blocks' && reassignBlocks.length > 0) {
